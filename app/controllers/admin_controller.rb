class AdminController < ApplicationController
  ...
  def index
    ...
    @quick_stats = {
      ...
      avg_sale: Order.where(created: Time.now.midnight..Time.now).avarage(:total).round(),
      per_sale: OrderProduct.joins(:order).where(orders: { created_at: Time.now.midnight..Time.now }).avarage(:quantity)
    }
    ...
    if @revenue_by_day.count < 7
      ...
      ordered_days_with_current_last = ordered_days_with_current_last.map { |day| [data_hash.fetch(day, 0)] }
